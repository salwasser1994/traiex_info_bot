...
# Клавиатура с вариантами ответа + кнопка назад в меню
...
async def handle_message(message: types.Message):
    ...
    elif message.text.startswith("А") or message.text.startswith("B") or message.text.startswith("A"):
        if user_id in user_state:
            q_index = user_state[user_id]
            correct_index = test_questions[q_index]["answer"]
            options = test_questions[q_index]["options"]

            if options[correct_index] == message.text:
                # правильный ответ → следующий вопрос
                q_index += 1
                if q_index < len(test_questions):
                    user_state[user_id] = q_index
                    options = test_questions[q_index]["options"]
                    await message.answer(test_questions[q_index]["question"], reply_markup=test_keyboard(options))
                else:
                    del user_state[user_id]
                    await message.answer("Тест завершён ✅", reply_markup=main_menu())
            else:
                # неправильный ответ → пока ничего не происходит
                pass
