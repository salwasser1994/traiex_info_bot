...
# Главное меню через ReplyKeyboard
def reply_main_menu():
...
async def cmd_start(message: types.Message):
    start_text = (
        ...
        "Хотите ли использовать новые инструменты: а именно искусственный интеллект + ежедневный сложный процент, или останетесь с одним источником дохода?"
    )
    await message.answer(start_text, reply_markup=show_menu_button())

# Обработка нажатия кнопки "Показать меню"
@dp.callback_query()
async def callbacks(callback: types.CallbackQuery):
    if callback.data == "show_menu":
        await callback.message.answer(reply_markup=reply_main_menu())
        await callback.answer()  # чтобы убрать "часик" на кнопке

# Пока кнопки меню без действия
@dp.message()
