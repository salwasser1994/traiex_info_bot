...
async def cmd_start(message: types.Message):
    text_content = (
        ...
        "Хотите ли использовать новые инструменты : а именно искусвенный интелект + ежедневный сложный процент. \n"
        "Или останетесь с одним источником дохода?"
    )
    await message.answer(text_content, reply_markup=show_menu_button())
...
async def callbacks(callback: types.CallbackQuery):
    ...
    await callback.answer()  # подтверждение нажатия, без сообщения

# Обработка нажатий кнопок ReplyKeyboard (пока без действия)
@dp.message()
async def handle_reply_buttons(message: types.Message):
    # Пока ничего не делаем при нажатии кнопок
    pass
