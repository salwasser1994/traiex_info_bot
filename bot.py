...
user_progress = {}
user_state = {}
user_data = {}   # данные для ИИ-теста (машина/дом/доход)
...
async def handle_message(message: types.Message):
    ...
    if message.text == "📝 Пройти тест":
    ...
    elif message.text in ["Машина", "Дом", "Пассивный доход"]:
        user_data[user_id]["path"] = message.text
    ...
    elif message.text.isdigit() and user_id in user_data:
        data = user_data[user_id]
        if "first" not in data:
            user_data[user_id]["first"] = int(message.text)
            kb = ReplyKeyboardMarkup(
        ...
        elif "second" not in data:
            user_data[user_id]["second"] = int(message.text)
            path = data["path"]
            first = data["first"]
            monthly = data["second"]
    ...
    # --- Остальные разделы ---
        ...
        await message.answer("Стоит отметить что таблица сделана на примерных цифрах...", reply_markup=main_menu())
