...
user_progress = {}       # старый тест
user_state = {}          # Общая картина
user_answers = {}        # новый тест
...
async def send_scenario_question(message: types.Message, user_id: int, step: int):
    if step == 0:
        ...
        keyboard = ReplyKeyboardMarkup(
            keyboard=[[KeyboardButton(opt)] for opt in question["options"]] + [[KeyboardButton("⬅ Назад в меню")]],
            resize_keyboard=True
        )
...
async def handle_message(message: types.Message):
    ...
    if message.text == "📊 Общая картина":
        ...
        text = "Чтобы увидеть всю финансовую картину целиком..."
        ...
        text = "Вот почему так важно видеть всю картину целиком."
    ...
    # --- Старый тест ---
        ...
        await message.answer("📘 Инструкция:\nВыберите правильный ответ.", reply_markup=start_test_menu())
    ...
    # --- Новый тест ---
        ...
        # шаги 1 и 2
            ...
            if message.text in [opt for q in scenario_questions[scenario] for opt in q["options"]]:
                answers.append(message.text)
                user_answers[user_id] = answers
                if len(answers) == 3:
                    # Расчёт сложного процента 135% годовых
                    target_amount = int(answers[1].replace("₽","").replace(" ","")) if scenario != "Пассивный доход" else int(answers[1].replace("₽","").replace(" ",""))
                    monthly_invest = int(answers[2].replace("₽","").replace(" ",""))
                    annual_rate = 1.35
                    monthly_rate = annual_rate**(1/12)-1
                    months_needed = math.ceil(math.log(1 + (target_amount*monthly_rate)/monthly_invest)/math.log(1 + monthly_rate))
                    item_name = "вашу цель" if scenario!="Пассивный доход" else "желаемый пассивный доход"
                    text = (
                        f"📊 Подробный расчёт:\n\n"
                        f"Вы хотите накопить: {answers[1]}\n"
                        f"Ваш ежемесячный вклад: {monthly_invest} ₽\n"
                        f"Средняя доходность бота: 135% годовых (сложный процент)\n\n"
                        f"С помощью нашего ИИ-бота, при ваших инвестициях {monthly_invest} ₽ в месяц, "
                        f"вы сможете достичь {item_name} через {months_needed} месяцев."
                    )
                    await message.answer(text)

                    keyboard = ReplyKeyboardMarkup(
                        keyboard=[[KeyboardButton("💰 Готов инвестировать"), KeyboardButton("не готов")]],
                        resize_keyboard=True
                    )
